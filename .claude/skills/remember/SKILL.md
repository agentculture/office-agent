...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope office-agent
...
scope:** the default personal scope is **private** (`--scope office-agent
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `office-agent`). Private records are served only to a recall in the same
