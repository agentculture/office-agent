...
**Watch for the format-confirm dialog.** When saving back to a
`.svg` that Inkscape didn't originate (e.g. one generated by
`office floors scaffold` or doctored), Inkscape sometimes pops a
"keep as Plain SVG?" confirmation. **Click "Use Plain SVG"** —
dismissing the dialog (or hitting Escape) silently aborts the save
and the file on disk stays unchanged. If a subsequent
`office floors validate` reports the file as untouched, this
dialog is the usual cause.
